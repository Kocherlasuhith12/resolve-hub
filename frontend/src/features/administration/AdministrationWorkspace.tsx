...
export function AdministrationWorkspace({ organisationId }: { organisationId: string }) {
  ...
  const [activeTab, setActiveTab] = useState<'lifecycle' | 'analytics' | 'developer'>('lifecycle')
  ...
  const canUpdateMember = permissions.has('member:update')
  ...
  const canManageIntegrations = permissions.has('apikey:manage') || permissions.has('webhook:manage')
  const isAdministrator =
    canInvite || canCreateDepartment || canCreateCategory || canManageSla || canReadAnalytics || canManageIntegrations

  const roles = useQuery({
    queryKey: ['roles', organisationId],
    queryFn: () => request<Role[]>(`/organisations/${organisationId}/roles`),
    enabled: canInvite || canUpdateMember,
  })
  ...
  const updateMemberRole = useMutation({
    mutationFn: (payload: { memberId: string; roleId: string }) =>
      request<Member>(`/organisations/${organisationId}/members/${payload.memberId}/role`, {
        method: 'PATCH',
        body: JSON.stringify({ role_id: payload.roleId }),
      }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['members', organisationId] })
    },
  })

  const updateMemberStatus = useMutation({
    mutationFn: (payload: { memberId: string; isActive: boolean }) =>
      request<Member>(`/organisations/${organisationId}/members/${payload.memberId}/status`, {
        method: 'PATCH',
    ...
    <section className="administration-workspace" aria-labelledby="administration-title">
      <div className="admin-header">
        <div className="page-header">
          ...
          <p className="page-subtitle">Manage team, services, and policies</p>
        </div>
        <span className="badge badge-status" style={{ background: 'var(--green-50)', color: 'var(--green-700)' }}>
          {membership.data.role_name}
        </span>
      </div>

      <div className="admin-tabs">
        <button
          className={`admin-tab-btn ${activeTab === 'lifecycle' ? 'active' : ''}`}
      ...
      {activeTab === 'analytics' && <AnalyticsTab organisationId={organisationId} />}

      {activeTab === 'developer' && <DeveloperTab organisationId={organisationId} />}
      ...
      {activeTab === 'lifecycle' && (
        <>
                  ...
                  {(members.data ?? []).map((member) => (
                    <li key={member.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span>
                        ...
                        <small style={{ display: 'block' }}>{member.email}</small>
                      </span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        {canUpdateMember ? (
                          <select
                            defaultValue={member.role_id}
                            onChange={(e) => updateMemberRole.mutate({ memberId: member.id, roleId: e.target.value })}
                        ...
                        {canUpdateMember ? (
                          <button
                            ...
                            onClick={() => updateMemberStatus.mutate({ memberId: member.id, isActive: !member.is_active })}
                            disabled={updateMemberStatus.isPending}
                          >
                            {member.is_active ? 'Deactivate' : 'Activate'}
                          </button>
                        ) : (
                          <span>{member.is_active ? '' : ' · Inactive'}</span>
                        )}
                      </div>
                    </li>
                  ))}
                </ul>
                {(updateMemberRole.isError || updateMemberStatus.isError) && (
                  <div className="form-error" role="alert" style={{ marginTop: '0.5rem' }}>
                    Member status/role change could not be saved.
                  </div>
                )}
              </div>
                      ...
                      <li key={invitation.id}>
                        ...
                        <span className="invitation-actions">
                          ...
                          {invitation.status === 'PENDING' && <>
                            <button className="btn-ghost btn-sm" type="button" aria-label={`Resend invitation for ${invitation.email}`} disabled={resendInvitation.isPending} onClick={() => resendInvitation.mutate(invitation.id)}>Resend</button>
                            <button className="btn-ghost btn-sm danger-text" type="button" aria-label={`Revoke invitation for ${invitation.email}`} disabled={revokeInvitation.isPending} onClick={() => revokeInvitation.mutate(invitation.id)}>Revoke</button>
                          </>}
                        </span>
                      </li>
                    ))}
          ...
          {canManageSla && (
            <section className="admin-panel" aria-labelledby="sla-admin-title">
              <div>
